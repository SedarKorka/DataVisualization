...
newDataForNonCauminicate<-Noncauminicate %>% 
                                            select(region,Year, Number, Age_Group,Age_group_code)  %>% 
...
mapa_animado_4<-ggplot(data=mapdata_join, aes(long, lat,  group= group,  fill= Number)) +
  ...
  scale_fill_viridis(option = "B", name= "Years",
                     guide = guide_colorbar(
                       ...
                       title.vjust = 0.5,
                       na.value = "white"))  +
  labs(title="Comparing Mortality Rates of 70+ Population in Luxembourg to Those of Other Countries to Understand the Impact of Non-Communicable Diseases.",
